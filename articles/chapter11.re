...
それぞれの作業は作業時間を必ず定義して、時間を測って実施します。最初は作業にかかる時間が不透明なため、感覚で設定することになります。予定通りに終わらなくても問題ありません。実際、私たちのチームも最初は作業時間をかなり過小評価してしまいました。次のスプリントで作業時間を修正すれば問題ありません。作業時間を設定すること自体が重要だと考えています。スプリントを繰り返すうちに、段々と作業時間の見積もりが最適化されていきます。後述するふりかえりの詳細な手順で私たちが設定した作業時間を示します。この時間は参考情報であり、チームメンバーの属性や能力、コンディションによって時間は変動しますので、スプリントを通じて最適な時間を見つけていくことが重要です。
...
（仕事、趣味なんでもOK）を２つ上げましょう。最後に感情を添えてください。
