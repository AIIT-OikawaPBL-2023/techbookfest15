...
私たちのスクラムでは、スプリントバックログアイテムをさらにタスクレベルまで分解し、「スプリントタスク」という別リストにして管理しています。チームで決めた「完成の定義」を満たすインクリメントを作るのに必要なタスクを列挙し、それぞれについて作業時間を見積もります。作業時間の見積もりには「見積もりポーカー」の手法を用いています（無料ツールの Hatjitsu<@<href>{https://hatjitsu.toolforge.org}>を活用しています）。各タスクに何ポモドーロ（第６章参照）必要そうかを各自が選択し、それを一斉に見せ合い、外れ値を出したメンバーの意見を聴き、チームとしての見積もりをすり合わせていきます。見積もりの結果、作業量が大きいと判断されたタスクは、１日（５ポモドーロ）で終わるタスクに分割します。
