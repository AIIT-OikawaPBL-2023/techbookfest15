...
１スプリントの長さ（タイムボックス）は一般的には「１か月以下」とされていますが、私たちは２週間としました。その理由として、プロダクト開発に使える時間が半年少々しかなく、１スプリント１か月とするとスプリント数が少なくなりすぎること、および、全員がスクラムの役割を２回ずつローテーションするには14スプリント必要だったことがあります。

== スプリントプランニング

スプリントプランニングは、各スプリントの初めに、そのスプリントで実施する作業の計画をスクラムチーム全員で議論して決めるイベントです。

所要時間（タイムボックス）は、スクラムガイドでは「スプリントが１か月の場合、最大8時間。スプリントの期間が短ければ、スプリントプランニングの時間も短くすることが多い。」とされていますが、私たちのスクラムでは内容を絞り込み、１時間～１時間半程度で実施しています。

===[notoc] スプリントプランニングの流れ
...
//image[Backlog][私たちのスプリントプランニングの流れ]


以下、各ステップについて順に説明します。

===[notoc] スプリントプランニングの詳細
...
私たちの多くはフルタイムの仕事を別途持っており、毎日デイリースクラムを行うことが現実的ではないため、毎日ではなく週３回のコアミーティングの冒頭15分間を利用してデイリースクラムを行っています。
...
スクラムガイド上、必須のイベントなのですが、私たちは「スプリントレビュー」という独立したイベントを実施していません。その理由としては、①主担当教員とのコアミーティングが週１回、副担当教員も含めたマンスリーレビューが月１回あり、これらの中で現在の方向性の共有やプロダクトのデモをし、方向性の修正をしていること、②我々が作るプロダクトのユーザは私たち自身であるため顧客からのフィードバックを常に受けられる状態で開発していること、および、③これらの状況があるにもかかわらずスプリント毎（２週に１回）にスプリントレビューを開催すると、限られた開発時間をいたずらに削ることにつながりかねないためです。

独立したイベントとしてのスプリントレビューを実施しなくとも、上記①②によってスクラムガイドが求めるスプリントレビューの内容は網羅されていると考え、私たちはこのような運用としました。

== スプリントレトロスペクティブ

スプリントレトロスペクティブとは、１スプリントの最終日にチーム全員でふりかえりを行うイベントです。ふりかえりの進め方は『アジャイルなチームをつくる ふりかえりガイドブック 始め方・ふりかえりの型・手法・マインドセット』@<fn>{retro} を参考に実施しています。以下この本のことを「ふりかえり本」と呼ぶことにします。
...
ふりかえりの作業は基本的に個人で書き出す作業と共有する作業の２つの作業の繰り返しで実施します。すなわち、個人でふせんに内容を記す「個人作業」とチームでふせんに書いた内容を共有する「共有作業」の２つをワンセットで行っていきます。

==== 作業時間の設定

それぞれの作業は作業時間を必ず定義して、時間を測って実施します。最初は作業にかかる時間が不透明なため、感覚で設定することになります。予定通りに終わらなくても問題ありません。実際、私たちのチームも最初は作業時間をかなり過小評価してしまいました。作業時間を設定すること自体が重要だと考えています。スプリントを繰り返すうちに、段々と作業時間の見積もりが最適化されていきます。後述するふりかえりの詳細な手順で私たちが設定した作業時間を示します。この時間は参考情報であり、チームメンバーの属性や能力、コンディションによって時間は変動しますので、スプリントを通じて最適な時間を見つけていくことが重要です。
...
たとえば、アイスブレークの事前の準備の例を示します。左上に示した「１」の部分にアイスブレークのグループワークで実施する内容を記しています。また、「２」の部分にメンバーのアイコンと空のふせんを置いておきます。ここまで準備することで、メンバーはやることが分からなくなったり、ふせんを作成する手間や、どのふせんを使用するか迷う手間が省けて書くことに集中できます。

Miroなどオンラインツールを利用する利点の１つに、事前に準備した内容を再利用できる点が挙げられます。事前の準備で一度作ったテンプレートをコピー&ペーストすることで次のスプリントのふりかえりの準備がすぐ整います。オフライン実施ではふりかえりを実施するとふせんを移動するなどするため、事前に準備した内容が記録に残りづらいです。一方でオンライン実施ではテンプレートという形で事前の準備内容が記録に残るため、そのようなふりかえりのテンプレートの品質もスプリントを重ねるごとに高まっていきます。
...
スプリント期間中にあった、PBL以外の出来事（仕事、趣味なんでもOK）を２つ上げましょう。最後に感情を添えてください。
...
事前の準備でスクラムマスターが用意したふせん２枚に好きなことを記して共有します。

//image[furikaeri3][アイスブレークを実施する]

==== ふりかえりのルールを確認する

１回目のふりかえりでルールを作成し、以降のふりかえりでは必要に応じてルールを確認しました。ふりかえりのルールを確認するフェーズで実施したグループワークを下記に記します。DPA手法の詳細は「ふりかえり本」を参照してください。
...
手法：DPA (Design the Partnership Alliance)
 ...
 １. どんな雰囲気でふりかえりを進めたいか決める。(１or２個)

 * 全員が書いたふせんを共有して「全員が合意できる」ものだけふせんにマーク
 * 個人作業２分 共有と合意３分

 ２. その雰囲気を作り出すために何をするか決める。(１or２個)
 ...
 * 「やったことを書き出す」で共有した内容を元に書き出す。
 * 「やったことを書き出す」で議論して学んだこと、気付いたことを書いてもOK。
 ...
 * すぐにできる。または期限が決まっている。：Timely / Time-bounded
...
時間：５分（制限時間または意見が出なくなったところで打ち切り）
 ...
 * 成長を感じる
 * アクションの棚卸しをやってみたら自分たちの活動に対して一段メタ的な視点でふりかえりができた
 ...
 * ２週間のうちにやったことを結構忘れてしまっていることがわかった。ふりかえりが定期的に開催されることで、ここまで自分ができた。ここまでやった。という実感が湧きやすくなる
