...
PBL活動の中で一定のウェイトを占めるプロダクト開発でモブプログラミングを行っています。また、開発以外にもメンバー全員が一堂に会する機会であることから、新しい知識の獲得であったり、開発におけるトラブルを一人で解決できなかった場合のトラブルシューティングとして活用したりすることもあります。本章ではこれらのチーム活動を「モブプロ」、一般的なモブプラグラミングをカギ括弧無しのモブプロとして総称することにします。ここでは「モブプロ」についての概要、やってよかった点に加え、苦労した点についても紹介します。
...
物理的に１つのキーボードを交代で使用するモブプロに近い体感をオンライン上で実現するために、Visual Studio Codeの拡張機能のLiveShare(@<href>{https://marketplace.visualstudio.com/items?itemName=MS-vsliveshare.vsliveshare})をメンバー全員がインストールして、タイピスト役のエディタに他メンバーが参加できるようにしました。LiveShare以外にもTODO Tree(<@<href>{https://marketplace.visualstudio.com/items?itemName=Gruntfuggly.todo-tree})という拡張機能でモブプロ時のみ使うコメント (例えば"MOB プログラムを動かす"など) をわかりやすく管理する工夫を導入しています。
...
たとえばSlack Botで使いたい機能はドキュメントのどこに書いてあるのか、という点について話し合った後で、ドキュメントの章立てからどこにどういう内容が書いてあるのかをより深掘りするような流れになったことがありました。
...
PBL開始当初からチームで物事を進めていくことに重点を置いていたので、「わいわい」というキーワードはしばしばスプリントレトロスペクティブ（@<hd>{chapter11|スプリントレトロスペクティブ}参照）などでもメンバーの口から出るキーワードでした。「モブプロ」の場はまさにこの点を体現するような取り組みを総合的に行っている場だと感じられます。
